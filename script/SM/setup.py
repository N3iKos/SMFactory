...
def SM_Script(WEBUI):
    return [
        f'{SMF_RAW}/script/SM/venv.py {WEBUI}',
        f'{SMF_RAW}/script/SM/Launcher.py {WEBUI}',
        f'{SMF_RAW}/script/SM/segsmaker.py {WEBUI}'
    ]

def CN_Script(WEBUI):
    return [
        f'{SMF_RAW}/script/controlnet.py {WEBUI}/asd',
        f'{SMF_RAW}/script/cn15.py {WEBUI}/asd',
        f'{SMF_RAW}/script/cnxl.py {WEBUI}/asd',
    ]
...
def tmp_cleaning(v):
    for i in TMP.iterdir():
        if i.is_dir() and i != v:   shutil.rmtree(i)
        elif i.is_file() and i != v: i.unlink()
...
def marking(p, n, i):
    ...
    if not t.exists():
        t.write_text(json.dumps({
            'ui': i, 'launch_args': '', 'zrok_token': '', 'ngrok_token': '', 'tunnel': ''
        }, indent=4))
...
def parallel_clone(urls, dest_dir, max_workers=5):
    ...
    GREEN_    = '\033[38;5;35m'; RED_ = '\033[31m'; RESET_ = '\033[0m'
...
def parallel_download_list(items, max_workers=4):
    """Fan out download() calls concurrently."""
    if not items: return
    def _do(item):
        try:
...
def webui_req(U, W, M):
    ...
    # Scripts are small — fire them sequentially
    for item in scripts: download(item)

    # Upscalers run concurrently to speed up setup
...
def WebUIExtensions(U, W, M):
    ...
    if U == 'ComfyUI':
        ...
        ext_list = W / 'asd/custom_nodes.txt'
        if ext_list.exists():
            node_urls = [l.strip() for l in ext_list.read_text().splitlines()
        ...
        ext_list = W / 'asd/extension.txt'
        if ext_list.exists():
            ext_urls = [l.strip() for l in ext_list.read_text().splitlines()
                        if l.strip() and not l.strip().startswith('#')]
...
def installing_webui(U, W):
    ...
    E = (
        ...
        else M / 'embeddings' if U in ['Forge-Classic', 'ComfyUI']
        else W / 'embeddings'
    )
    ...
    with output:
        ...
        if ui == 'FaceFusion':
            ...
            req = [
                ...
                f'mkdir -p {WEBUI}/dataset', f'mkdir -p {WEBUI}/VAE',
...
def Segsmaker_Setup_Widgets():
    for cmd in [
        f'curl -sLo {CSS} {SMF_RAW}/script/SM/setup.css',
        f'curl -sLo {IMG} {SMF_RAW}/script/loading.png',
        f'curl -sLo {MRK} {SMF_RAW}/script/marking.py'
    ]: SyS(cmd)
